...
CREATE TRIGGER event_summary_index_queue_delete AFTER DELETE ON event_summary
  FOR EACH ROW EXECUTE PROCEDURE insert_queue_summary_delete();

CREATE TRIGGER event_archive_index_queue_insert AFTER INSERT OR UPDATE ON event_archive
...
CREATE TRIGGER event_archive_index_queue_delete AFTER DELETE ON event_archive
  FOR EACH ROW EXECUTE PROCEDURE insert_queue_archive_delete();
